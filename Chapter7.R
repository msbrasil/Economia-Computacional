...
l: lines
b: both (p and l)
c: like b, but no points
o: like b, but overplotted
h: histogram
n: no plotting
'''
...
points(x[(x >= 5 & x <= 15) & (y > -5 &
                                 y < 5)], y[(x >= 5 &
                                               x <= 15) & (y > -5 & y < 5)],
       pch = 19, col = "blue")

points(x[(x < 5 | x > 15) & (y > -5 & y < 5)], y[(x < 5 |
                                                    x > 15) &
                                                   (y > -5 & y < 5)])
...
points(weight[sex == 'female'], height[sex == 'female'], pch = 4)
...
qplot(foo, bar, geom = 'blank') + geom_point(size = 3,
                                             shape = 6,
                                             color = 'blue') +
...
myqplot <-
  qplot(foo, bar, geom = 'blank') + geom_line(color = 'red', linetype = 2)
myqplot + geom_point(size = 3,
                     shape = 3,
                     color = 'blue')
myqplot + geom_point(size = 3,
                     shape = 7,
                     color = 'green') #shape <= 25
